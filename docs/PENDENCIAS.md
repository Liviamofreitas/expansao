...
| **A13** | ~~17 pares (contrato, tipo, evento) com duas regras de alvo CONTRATO.~~ **Medido campo a campo: 12 eram duplicatas exatas e 5 são conflito real — e os 5 são a MESMA pergunta, feita a 2 clientes.** | **Reduzida a 1 pergunta.** O motor não trata mais duplicata como ambiguidade (caso normativo `MAT-16`, referência Python e porte Java em acordo), e a recusa nomeia a divergência: `prazo (INICIO_COMPETENCIA +1 UTIL e INICIO_COMPETENCIA +10 UTIL)`. Ver achados §58. | **1 decisão:** o comprovante de pagamento da rescisão (`RES.COMPROVANTE_PG \| RESCISAO`) vence no **1º ou no 10º dia útil**? Para CEF e TJ CE. É cadastro, não release. **Ainda bloqueia a Onda 1** — junto com A14. |
...
| ~~**RA-03**~~ | ~~O peso do alias no score (`PESO_DO_ALIAS = 0,10`) estava no código, não no cadastro.~~ | — | **Fechada.** O peso vem de `parametro`, chave `classificacao.peso_do_alias`, escopo GLOBAL (V107). **A ausência não vira padrão em código** — cair num 0,10 escrito no Java recriaria a RA-10 —, desliga o bônus e diz por quê; zero cadastrado é um fato diferente de nenhum cadastro; fora da faixa e texto inválido **recusam**, porque um peso de 0,5 faria o nome do arquivo classificar sozinho. Ver achados §56. |
| ~~**RA-19**~~ | ~~O `RepositorioDeAlias` não estava ligado ao pipeline: a varredura passava `null` no lugar do bônus, e nenhum arquivo ingerido passava por ele.~~ | — | **Fechada.** A varredura carrega o de-para uma vez por lote e aplica o bônus por arquivo. Medido sobre dado real: conteúdo 0,75 → confiança gravada 0,85 — **e o documento continua indo à triagem**, que é a prova de que o alias corrobora e não elege. Bônus desligado **viaja no resultado** e não conta como incompletude. Ver achados §57. |
...
| **RA-07** | Nada roda sozinho: a abertura de ciclo, a varredura, a derivação de eventos, as regras de conciliação e a régua de notificação existem, são idempotentes e têm teste — mas **nenhum agendador as chama**. | **Metade fechada.** O mecanismo existe (V018 + `Agendador`): lock consultivo por job, registro de toda tentativa, alerta de job silencioso e de job travado, e a recusa estrutural de agendar a derivação de eventos (cap. 7.2). Um agendador morto agora é NOTADO — antes era indistinguível de "não havia o que fazer". Ver achados § 46. | Os gatilhos serem ligados. Continua gated na **F3-01**: pôr o sistema a cobrar áreas antes de a divergência estar medida é o risco P01 do cap. 22. O que falta é decisão de governança, não código. |
...
| ~~**RA-10**~~ | ~~`LeitorDeFopag` carregava os nove códigos estruturais em constante Java, embora os mesmos nove já existissem em `rubrica_de_para`.~~ | — | **Fechada.** O leitor recebe o de-para do cadastro no construtor e **recusa sem ele**; faltando papel estrutural, não há leitura (ler produziria nulo em todos os colaboradores e a conciliação compararia contra nada). Dois códigos para o mesmo papel são **recusados, não desempatados**. Há teste que amarra o leitor real à carga real — tirar uma linha do seed derruba a suíte — e um que prova que o leitor segue o cadastro e nada mais. Ver achados §54. |
| **RA-18** | **O `VALE_ALIMENTACAO` da carga é ambíguo:** `17300` (custo total — empresa + coparticipação) e `17305` (custo da empresa) declaram o mesmo papel. Uma regra que pergunte "qual é o VA?" tem duas respostas, e escolher errado é divergência de valor num book que alguém vai atestar. | A leitura por papel **recusa** em vez de escolher (`RubricaAmbigua`), e um teste nomeia os dois códigos. Nada consome o papel VA hoje, então nada quebra. | **Decisão de cadastro/contrato:** contra qual dos dois a conciliação do VA compara. Se forem os dois, o papel precisa ser dividido em `VALE_ALIMENTACAO` e `VALE_ALIMENTACAO_EMPRESA` — migração no CHECK da V014 e uma linha no seed. Não é engenharia: é leitura de contrato. |
...
| ~~**RA-12**~~ | ~~A cláusula `WHERE` que repete a pré-condição de transição dentro do `UPDATE` não estava medida: removê-la não derrubava nenhuma das 57 asserções.~~ | — | **Fechada.** A janela se reproduz sem pausar o código de produção: uma terceira conexão segura o lock da linha, o chamador lê sem bloquear, passa pela guarda de Java e **para no `UPDATE`**; dentro da janela outra sessão muda o ciclo e comita. Ponto de sincronização pelo `pg_stat_activity`, não por sleep. Sem a cláusula, **4 asserções caem**. Uma quinta sobrevive à quebra e está documentada como cega. Ver achados §55. |
...
| **RA-15** | O `ObservadorDeAcesso` não era exercitado pela suíte, e `RegistroDeAcesso.observar` devolvia `false` tanto para "já registrada" — o caso normal — quanto para "falhou". **O mesmo valor para dois fatos opostos:** uma falha persistente de escrita produzia relatório de recertificação incompleto **sem sintoma**. | **Fechada na maior parte.** O desfecho agora é nomeado (`GRAVADA`/`JA_REGISTRADA`/`SEM_CONCESSAO`/`FALHOU`), a falha é contada e deixa **duas marcas** — contador do processo e linha na trilha (`acao = OBSERVAR_ACESSO`, `resultado = ERRO`, uma por dia). A ressalva do relatório passa a dizer que a lista está **INCOMPLETA e não deve ser aprovada**. O interceptor ganhou teste: 4xx, 5xx, ator nulo, caminho feliz e as duas falhas que não podem subir. Ver achados §53. | **Resta uma metade:** o contador reinicia com o processo e não atravessa instâncias, e é ele — não a trilha — que a ressalva lê. Fechar por inteiro é ler a trilha ao montar o relatório. Engenharia. |
| **RA-16** | A recertificação cobre o acesso **observado**: uma conta que existe no diretório e nunca entrou no SGDF não aparece no relatório. É justamente a conta dormente que uma revisão de acessos existe para achar. | A ressalva é a primeira linha do relatório, em CSV e em JSON, e diz que a revisão do SEC-10 se completa comparando esta lista com a de contas concedidas. Aprovar só esta metade não cumpre o controle, e o documento afirma isso. | Leitura do diretório (SCIM, LDAP ou export periódico do provedor). **Não se resolve no SGDF** sem uma integração que hoje não existe e que é decisão da DAF e da TI. |
| **A13** | **17 pares (contrato, tipo, evento) com duas regras de alvo CONTRATO** na versão 1.1 da matriz. O `Resolvedor` recusa com `REGRA_AMBIGUA` — e recusa certo: escolher uma das duas em silêncio aplicaria prazo ou criticidade que ninguém decidiu. | A abertura desfaz o ciclo que não materializa nada, então o painel não mostra um ciclo vazio como completo. Mas o contrato **não abre**. | **Decisão de cadastro:** qual das duas regras vale em cada par. É leitura do Anexo 1, não engenharia. **Bloqueia a Onda 1** — sem isto, nenhum ciclo abre. |
| **A14** | **Nenhum dos 12 contratos-serviço tem `empresa_id`.** A V004 exige empresa emitente para exigência de escopo corporativo, e há uma única empresa cadastrada: `PRESTADOR A CADASTRAR / 00000000000000`, um placeholder. | Mesma contenção do A13: o ciclo vazio é desfeito em vez de parecer completo. | **Decisão de cadastro:** qual empresa do grupo emite para cada contrato. **Bloqueia a Onda 1.** |
| **RA-17** | As 303 regras da carga são **todas de alvo CONTRATO**; zero de modalidade. O cap. 7.1 manda unir os blocos da modalidade às regras do contrato, e a carga produziu só o segundo termo. Um contrato-serviço novo materializa **zero** até alguém escrever regras para ele. | A abertura desfaz o ciclo vazio e nomeia o contrato na lista de falhas. | Pode ser deliberado — cada contrato tem o seu checklist no Anexo 1. Se for, o cadastro de contrato novo tem um passo obrigatório que nada no sistema exige nem lembra; se não for, faltam os blocos de modalidade. **Decisão da curadoria da matriz.** |
