...
| **RA-15** | O `ObservadorDeAcesso` — o interceptor que grava a observação de acesso — não é exercitado pela suíte: os testes de fronteira chamam os controladores diretamente, sem passar por interceptor. E `RegistroDeAcesso.observar` engole a exceção de propósito, para que a recertificação não derrube o produto que ela revisa. | Toda a decisão (o que vale como observação, a ordenação dos arrays, a idempotência do dia) está no `RegistroDeAcesso`, exercitado contra o banco em 33 asserções. O interceptor tem cinco linhas de adaptação. | Um teste de integração que suba o contexto Spring, ou um contador de falhas de observação exposto como indicador — hoje uma falha persistente de escrita produz relatório incompleto **sem sintoma**. Engenharia. |
