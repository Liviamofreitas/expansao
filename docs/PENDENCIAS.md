...
| **RA-19** | O `RepositorioDeAlias` **não está ligado ao pipeline**: só o teste o constrói. O bônus de alias existe, vem do cadastro e é medido — e nenhum arquivo que a varredura ingere passa por ele. | Nenhuma: o efeito é que a F1-06 não cumpre "o mesmo padrão não retorna" em produção, mesmo com o alias gravado. Registrado em vez de suposto pronto. | Ligar o bônus ao `Classificador` dentro do pipeline de ingestão. Engenharia, pequena. |
