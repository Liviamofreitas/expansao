...
| **RA-07** | A abertura de ciclo (cap. 7.6) ainda não tem agendador: `RepositorioDaMatriz.materializar` existe e é idempotente, mas alguém precisa chamá-lo. | Nada dispara sozinho, então nada acontece errado sozinho. A materialização é reexecutável sem duplicar. | O endpoint/agendador da F0-07 ser ligado. Engenharia — a regra e a persistência estão prontas e cobertas pelas suítes normativas. |
